...
def main():
    ...
    # create a list of 3 clustering matrices
    Hist1ClusterPercentages = [[] for _ in range(len(bestCentroidGroupings))]

    # loop through the 3 clustering matrices
    for index, cluster in enumerate(clusteringMatrices):
        ...
        for x in range(len(cluster)):
            ...
            for y in range (len(cluster[x])):
                ...
                if cluster[x][y] == 1 and headerColumns["Hist1"][y] == "1":
                    hist1Counter += 1

            percent = round((hist1Counter / len(cluster[x])) * 100, 2)
            Hist1ClusterPercentages[index].append(percent)

    plt.title("Hist1 Features")
    plt.xlabel("Cluster")
    plt.ylabel("% each NP which detected a window")
    plt.boxplot(Hist1ClusterPercentages, showmeans=True)

    for index in range(len(Hist1ClusterPercentages)):
        y = Hist1ClusterPercentages[index]
        x = [index + 1] * len(y)
    ...
    LADClusterPercentages = [[] for _ in range(len(bestCentroidGroupings))]

    # loop through the 3 clustering matrices
    for index, cluster in enumerate(clusteringMatrices):
        # loop over the rows in the matrices
        for x in range(len(cluster)):
            LADCounter = 0
            ...
            for y in range (len(cluster[x])):
                ...
                # if both the cluster at [row][col] is 1 (the heatmap) and it's a LAD in the feature table
                if cluster[x][y] == 1 and headerColumns["LAD"][y] == "1":
                    LADCounter += 1

            percent = round((LADCounter / len(cluster[x])) * 100, 2)
            LADClusterPercentages[index].append(percent)

    plt.title("LAD Features")
    plt.xlabel("Cluster")
    plt.ylabel("% each NP which detected a window")
    plt.boxplot(LADClusterPercentages, showmeans=True)

    for index in range(len(LADClusterPercentages)):
        y = LADClusterPercentages[index]
        x = [index + 1] * len(y)
        plt.scatter(x, y, alpha=0.3)
    plt.show()

    for cluster in Hist1ClusterPercentages:
        for percentage in cluster:
            print(f"{percentage}%")
